...
- ⚠️ **Il deploy NON applica le migration** (`deploy/deploy.sh` lo dice esplicitamente): vanno pushate a mano **prima** di mandare il codice in produzione. Nell'ordine inverso il backend nuovo scrive su colonne inesistenti e le rotte coinvolte iniziano a fallire. Nota che un backend **vecchio** invece ignora in silenzio i campi che non conosce (Fastify li scarta se non sono nello schema): la richiesta risponde 200 e il dato non viene mai salvato, senza errori da nessuna parte. Se un campo nuovo "non si salva" in produzione, controllare `git log master..develop` prima del codice applicativo.
...
- **Schede preconfezionate** (`workout_templates`): libreria di programmi pronti (stessa forma `days_json` delle schede) NON legati a un member. Trainer/admin le gestiscono e le **assegnano** a un cliente via `POST /api/templates/:id/assign`, che ne fa una **copia** in `workouts`. Vista `views/trainer/TemplatesView.vue`, rotta `backend/src/routes/templates.js`, seed di ~8 programmi noti (PPL, Full Body, 5x5, Arnold split…).
